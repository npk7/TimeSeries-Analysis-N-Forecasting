...
proc glm data = auto2;
where foreign=1;
model weight length = price mpg rep78 hdroom trunk turn displ gratio; 
run;

proc reg data=auto2;
where foreign =1;
model mpg = length weight price rep78 hdroom trunk turn displ gratio/ selection = rsquare;
run;

proc glm data=auto2;
model length = price mpg rep78 hdroom trunk turn displ gratio;
run;

proc reg data=auto2;
model length weight = price mpg rep78 hdroom trunk turn displ gratio;
run;


proc reg data=auto2;
model weight price mpg rep78 hdroom trunk turn displ gratio =length;
run;

proc anova data=auto2;
where foreign=1;
model length = price mpg rep78 hdroom trunk turn displ gratio / intercept cli clm;
/*means price / scheffe;*/
run;

proc ttest data=auto2;
  class foreign;
  var mpg;
run;
title 'Summary Table';
proc summary data=auto2;
	class rep78 foreign;
	output out=auto2summary
...
proc glm data=auto2;
      ...
      where foreign=1;
      model length = price mpg rep78 hdroom trunk turn displ gratio / solution;
run;

proc glm data=auto2;
      ods select ParameterEstimates;
      where foreign=1;
      model length = weight price mpg rep78 hdroom trunk turn displ gratio / solution;
run;
...
proc glm data=auto2 alpha=0.05;
where foreign =1;
model length= weight / solution ss3;
run;

proc glm data=auto2;
     model mpg = length weight foreign length*weight weight*foreign/ stepwise;
run;

proc reg data=auto2;
model mpg = length weight foreign rep78 trunk hdroom / selection=stepwise;
run; 
